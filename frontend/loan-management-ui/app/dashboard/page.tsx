        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          {(loanChart.length > 0 || collectChart.length > 0) && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {loanChart.length > 0 && (
                <BarChart data={loanChart} label="Monthly Disbursements" color="bg-green-500" valuePrefix="$" />
              )}
              {collectChart.length > 0 && (
                <AreaChart data={collectChart} label="Monthly Collections" color="#22c55e" valuePrefix="$" />
              )}
            </div>
          )}
        </div>
