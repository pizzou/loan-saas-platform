...
/** Matches backend Payment.remaining() — not included in JSON, so compute client-side. */
export function paymentRemaining(p: Pick<Payment, 'amount' | 'penalty' | 'paidAmount'>): number {
  const amt = p.amount ?? 0;
  const pen = p.penalty ?? 0;
  const paid = p.paidAmount ?? 0;
  return Math.max(amt + pen - paid, 0);
}
